...
!!! info "動作確認用の初期ユーザ"
    ...
    初期データ投入時([実習ルール](rules.md)参照)に作成されるログインユーザは次の通りです。
...
| ⑤ | startDate    | 加入日         | 日付           | 空欄不可、YYYY/MM/DD   |                                          |
| ⑥ | endDate      | 解約日         | 日付           | YYYY/MM/DD             | 加入日より前の日付の場合はエラー         |
| ⑦ | paymentMethod | 決済方法      | ドロップダウン |                        | 「クレジット」「銀行振込」のいずれか |
| ⑧ | submit       | 保存           | ボタン         | ―                      |                                          |
| ⑨ | cancel       | キャンセル     | ボタン         | ―                      |                                          |
| ⑩ | delete       | 削除           | ボタン         | ―                      | 編集モード時のみ表示する。               |
...
クリックすると確認ダイアログを表示する。「OK」が選択された場合のみ、表示中の加入者情報を削除する。「キャンセル」が選択された場合は何も行わない。

削除処理は `/member/delete` に POST で行う。正常に削除が完了したら、**加入者検索条件画面**に遷移し、タイトル下部に「削除しました。」とメッセージを表示する。
...
| ④ | endDate   | 適用終了日 | 日付       | YYYY/MM/DD                 | 運用開始日より前の場合はエラー  |
| ⑤ | submit    | 保存       | ボタン     | ―                          |      |
| ⑥ | cancel    | キャンセル | ボタン     | ―                          |      |
