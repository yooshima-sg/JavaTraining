...
4.  「請求データ」並びに「請求明細データ」テーブルに対して、有効な加入者情報（※[1.4.1][3]参照）と有効な料金情報（※[1.4.5][8]参照）を参照し、次のようにそれぞれのテーブルのレコード作成、追加する。
    - 請求データ\
      (データ件数は、有効な加入者情報の数に等しい)
      - 請求年月 ← 対象年月
    ...
    - 請求明細データ\
      (データ件数は、有効な加入者情報と有効な料金情報の数を掛け合わせた数に等しい)
      - 請求年月 ← 対象年月
...
「[1.3][4][作成処理][4]」を実行中に発生した例外は、基本的に継続不可能なエラーとして、次のように対応します。
...
<figure>
<img src="media/media/image2.svg" style="width:4.34837in;height:1.77457in" />
<figcaption><p>図 有効な加入者情報(緑色になっているパターン2,3,4,6 が有効)<br />
緑色になっているパターン2,3,4,6 が有効な加入者情報</p></figcaption>
</figure>
...
<figure>
<img src="media/media/image2.svg" style="width:4.34837in;height:1.77457in" />
<figcaption><p>図 有効な料金情報(緑色になっているパターン2,3,4,6 が有効)</p></figcaption>
</figure>
